...
| [Scene rendering](../../src/maya/core/scene.cpp), [shader](../../resources/shaders/metal/triangle.metal) | Direct draws reuse one uniform region; normals use the model's upper 3x3. | #996 explicit passes (done), #997 independent draw/frame data, #998 extraction and correct normal transforms. Nonuniform-scale lighting is not yet correct. |
| [RHI handles](../../include/maya/rhi/resource.hpp), [Metal device](../../src/maya/rhi/metal/metal_device.mm) | Integer resource IDs and device-wide cleanup; no individual retirement API. | #996 validated handles, explicit passes, and deferred retirement (done); #997 frame allocation and backpressure. |
