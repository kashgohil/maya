...
The source audit below is against commit `0b5eb8e`. Subsequent implementation is recorded separately: [#991 World storage](../world.md) adds World, typed asset-reference values, and the MayaWorld target; [#992 spatial operations](../spatial.md) adds validated local transforms, hierarchy transactions, derived world matrices, and independent camera calculations. [#993 assets](../assets.md) adds a project registry, typed version handles, shared residency leases, and mesh buffer release. [#994 properties](../properties.md) adds shared schemas, typed access, validation, and atomic component edits. [#995 scene persistence](../scene.md) adds the versioned scene format, validated loading into a separate World, and atomic file saves. [#996 graphics device](../rhi.md) adds descriptor-based resources, explicit render passes, separate surface presentation, validation, and deferred GPU retirement. #997 adds bounded frames in flight and per-frame upload memory for independent draw data. [#998 renderer](../renderer.md) adds RenderSnapshot extraction, camera views, offscreen targets, and presentation, replacing the legacy Scene and Material. [#999 editor shell](../editor.md) adds Dear ImGui behind the editor target, a docked viewport sized to its panel and display scale, an editor camera, and explicit UI/viewport input routing. [#1000 scene editing](../editing.md) adds undoable edits, selection, and unsaved-changes state keyed by persistent EntityId. The other future services remain contracts rather than existing APIs.
...
| Scene and Material (removed in #998) | Flat drawable list, owning mesh vector, raw mesh/texture references. | Replaced by #991 world/identity, #993 shared assets, #994 properties, #995 persistence, and #998 extraction. |
...
Storage layout, an ECS library, a physics library, a scripting language, an editor UI library, and a general job system are not selected by this issue. (#999 later selected Dear ImGui for the initial editor UI.) In particular, validated handles and component access rules must survive whichever storage strategy #991 selects.
...
| A parent has rotated nonuniform scale and a child is reparented | World affine matrices preserve inherited shear. A keep-world reparent requiring unrepresentable local shear is rejected atomically. Physics rejects unsupported ancestry. | #992 hierarchy/reparent tests; #998 normal transform tests (positive scale never flips winding). |
