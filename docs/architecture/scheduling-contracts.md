...
Input routing occurs before simulation. A pressed/released edge is consumed by one assigned tick, held state persists, and no-tick frames retain pending edges. UI-consumed events do not leak to gameplay. Losing focus clears held gameplay actions. Benchmark/replay input bypasses live device state and logs the assigned tick. Fixed stepping does not promise bit-identical physics across hardware, thread schedules, or library versions; exact capture may require recorded state/caches.
