...
`Mesh` now owns its vertex/index allocations, is noncopyable/nonmovable, and rolls back a partial upload. Final mesh destruction calls nonthrowing buffer-release methods on the device. Metal removes the handles from its application-owned resource map; normal command buffers retain resources used by encoded work. This follows Apple's [retainedReferences contract](https://developer.apple.com/documentation/metal/mtlcommandbuffer/retainedreferences). Releasing the application map's ownership therefore does not free a buffer still used by a command buffer. Queue shutdown drains submitted work before clearing native resources. A retained last submission may keep its references longer; these counts are distinct from catalog entries and CPU leases.

Device sessions expose an expiring lifetime guard. Metal invalidates it at shutdown and creates a fresh guard on initialize. Mesh draw/release skips an expired session, and a mesh provider from an old session refuses uploads; leases can safely be destroyed after the device itself. A nonempty old lease still owns its CPU value, but `mesh().valid()` and registry handle resolution fail after device shutdown. Create a new provider/registry for the new device session. Normal Engine teardown still destroys content and registries before the device. Custom GraphicsDevice backends must invalidate/renew their guard and implement the release contract; transitional default release methods retain device-wide ownership.

This is the minimum adapter needed for shared mesh ownership. Uniforms, textures, and pipelines still follow device-wide lifetime rules, and legacy RHI integer handles do not yet validate device/type/generation. General RHI resource retirement, explicit submissions, fences, and asynchronous resource lifetime tracking remain #996/#997. Metal's `resident_buffer_count()` reports application-owned buffer map entries, not native GPU memory or completion state.
